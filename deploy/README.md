...
`LEAD_TEMP_ROOT` имеет единственное допустимое значение `/tmp/kordev-leads`. Worker работает с read-only root filesystem; web временно сохраняет writable root для совместимости действующего legacy-редактора контента до его полного перевода на PostgreSQL. Compose монтирует в lead temp root отдельный ограниченный 96 MiB tmpfs с uid 1000 и mode 0700, сохраняя отдельный общий `/tmp` mode-1777. `scripts/validate-runtime-compose.sh` и container entrypoint отклоняют другой путь или неверные owner/mode.
