...
Ежедневная retention-задача удаляет истёкшие локальные записи заявок, приватные S3-сироты и истёкшие HMAC-бакеты rate limit. Заявки и S3 обрабатываются отдельными ограниченными пачками, а rate-limit backlog дренируется пакетами до 100 строк, не более 100 000 строк или 30 секунд за запуск. Глобальный лимит 1000 валидных попыток в час ограничивает скорость создания IP/phone-бакетов; если backlog остаётся, команда завершается с ошибкой, а systemd повторяет запуск через 5 минут. Журналы содержат только агрегированные счётчики.

`LEAD_S3_BUCKET` должен быть отдельным приватным бакетом без публичной раздачи объектов. Для Timeweb используйте endpoint `https://s3.twcstorage.ru`, регион из настроек бакета и `LEAD_S3_SSE=provider`: Timeweb отклоняет AWS-заголовок SSE-S3, поэтому приложение не отправляет его. `LEAD_S3_SSE=AES256` оставлен для S3-реализаций, которые поддерживают этот заголовок. При изменении опубликованного текста согласия увеличьте `LEAD_CONSENT_VERSION`, чтобы каждая новая заявка сохраняла точную принятую версию.
