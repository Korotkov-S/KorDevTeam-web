...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  return data({ seo, entries: entries.map(entry => ({ title: entry.title, excerpt: entry.excerpt, href: `${pathname}${entry.slug}/` })) }, { headers: documentHeaders });
}
export default function Catalog() {
  const { seo, entries } = useLoaderData<typeof loader>();
