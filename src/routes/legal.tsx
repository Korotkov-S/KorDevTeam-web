...
const policy = [
  ...
  ["Какие данные используются", "Форма обращения предусматривает имя и один способ связи. Описание задачи, компания, услуга, бюджет и вложения необязательны. Не указывайте в обращении данные, которые не нужны для обсуждения задачи."],
  ["Обработка и передача", "Обработка включает получение, запись, хранение, использование для ответа, уточнение и удаление данных. Для обработки обращения предусмотрена доставка в CRM и по электронной почте с повторной попыткой при сбое. Вложения хранятся в объектном хранилище. Доступ к данным предоставляется для работы с обращением."],
  ["Срок хранения", "Копия обращения и вложения на сайте хранятся 30 дней, после чего подлежат удалению. Дальнейшее хранение деловой переписки и договорных документов определяется целью общения и требованиями законодательства."],
