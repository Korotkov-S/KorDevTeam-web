...
async function readMetadata(root: string): Promise<z.output<typeof articleMetadata>[]> {
  try {
    const raw: unknown = JSON.parse(await readFile(path.join(root, "public", "content", "blog.ru.json"), "utf8"));
    const parsed = z.array(articleMetadata).safeParse(raw);
...
export async function loadArticleSources(
  ...
  return Promise.all(slugs.map(async articleSlug => {
    const item = metadataBySlug.get(articleSlug);
    if (!item) throw new Error("article_source_unknown_slug");
