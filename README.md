...
Установите Node.js версии `22.x`. Проверить установленную версию можно командой:

```bash
node -v
```

Если Node.js не установлен, поставьте его любым удобным способом:

- скачать установщик с официального сайта Node.js;
- или установить через `nvm`, если он используется на машине.
...
API-запросы к `/api/*` проксируются на `http://localhost:3001`, поэтому для админки и динамического контента нужно отдельно запустить сервер.
