...
databaseTest("local portfolio import publishes 26 indexable SSR case routes", { timeout: 120_000 }, async t => {
  ...
  assert.equal(result.inserted + result.updated + result.unchanged, 26);
  ...
  assert.equal(new Set(entries.map(entry => entry.seoTitle)).size, 26);
  assert.equal(new Set(entries.map(entry => entry.seoDescription)).size, 26);
