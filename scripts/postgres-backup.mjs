...
export async function backupDatabase(config, client, run = runCommand) {
  ...
  const manifest = { version: 1, createdAt: new Date().toISOString(), reason, dumpSha256: checksum(dump), publishedCounts: counts, migrations };
...
export async function restoreDatabase(config, run = runCommand, connect = async value => {
  ...
  if (manifest.version !== 1 || !manifest.publishedCounts || !Array.isArray(manifest.migrations) || checksum(path.join(directory, 'backup.dump')) !== manifest.dumpSha256) throw Error('Dump checksum or manifest mismatch');
  ...
  try {
    ...
    same(await migrationRows(client), manifest.migrations);
    await applyMigrations(targetUrl);
    for (const table of ['admin_users', 'content_entries', 'content_relations', 'content_revisions', 'media_assets', 'redirects', 'site_settings']) {
      if (!(await client.query('SELECT to_regclass($1) AS name', [`public.${table}`])).rows[0].name) throw Error('Restored schema verification failed');
    }
    same(await publishedCounts(client), manifest.publishedCounts);
