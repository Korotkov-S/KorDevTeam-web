...
[[ $# == 2 && "$1" =~ ^[a-f0-9]{40}$ ]] || fail 'Usage: build-content-release.sh EXACT_SHA TOOL_IMAGE:EXACT_SHA'
...
docker build --target content-release --build-arg "RELEASE_SHA=$1" -t "$2" "$REPO_ROOT"
